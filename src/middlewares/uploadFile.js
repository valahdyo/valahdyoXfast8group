  ...
  const fileFilter = (req, file, cb) => {
    if (file.fieldname === imageFile) {
      if (!file.originalname.match(/\.(jpg|JPG|jpeg|JPEG|png|PNG)$/)) {
        ...
        return cb(
          new Error("Only image file with jpg, jpg, and png format allowed"),
          false
        )
      }
    }
    ...
    uploadFile(req, res, (err) => {
      if (!req.file && !err) {
        if (req.method === "PATCH" && Object.keys(req.body).length === 0)
          return res
            .status(400)
            .send({ message: "Please select image file to upload" })
      ...
      if (err) {
        if (err.code === "LIMIT_FILE_SIZE") {
          return res.status(400).send({
            message: `Max image file to upload is ${maxSize / 1024} KB`,
          })
        }
      }
      next()
